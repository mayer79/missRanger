...
The aim of this vignette is to introduce the R package `missRanger` and to explain how to use it for multiple imputations.

`missRanger` uses the `ranger` package [1] to do fast missing value imputation by chained random forest, see [2] and [3]. Basically, each variable is imputed by predictions from a random forest using all other variables as covariables. The algorithm iterates multiple times over all variables containing missing variables until the average out-of-bag prediction error of the models start to increase again.

Why should you consider `missRanger` over other options like `mice` or `missForest`?

- It is fast.

- It is flexible and intuitive: E.g. typing `missRanger(data, . ~ 1)` would impute all variables univariately.

- It can deal with almost all kind of variables, even dates and times etc. are being supported.

- It combines random forest imputation with predictive mean matching. This generates realistic variability and avoids "new" values like 0.3334 in a 0-1 coded variable.


In the examples below, we will use just two functions:
...
We first generate a data set with about 15% missing values per column and fill them again by `missRanger`.
...
`missRanger` worked! Unfortunately, the new values look somewhat unnatural because of additional digits. If we would like to avoid this, we just set the `pmm.k` argument to a positive number. All imputations done during the process are combined with a predictive mean matching step, leading to more natural imputations and distributional properties of the resulting values:
...
Note that `missRanger` offers a `...` argument to pass options to `ranger` like `num.trees` or `min.node.size`. How would we use its extra trees variant with 50 trees?
...
Further note that `missRanger` does not rely on `tidyverse` but you can embed it into a `dplyr` pipeline (without `group_by`). Make sure to set `verbose = 0` in order to prevent output.
...
iris$random_date <- seq.Date(as.Date("1998-12-17"), by = "1 day", length.out = nrow(iris))
